...
function CreateTopNotification({ setIsCreated }) {
  ...
  const handelCreateTraningVideo = async ({ name, description }) => {
    try {
      setIsDisabled(true);
      const { data } = await axios({
        url: `${process.env.NEXT_PUBLIC_URL_BD}/api/top_notification/save`,
        method: "POST",
        data: {
          name,
          description,
      ...
      toastifySuccess(`Top Notification added successfully`);
      setIsDisabled(false);
      reset();
      setTimeout(() => {
        setIsCreated(false);
    ...
    <div>
      ...
      <form onSubmit={handleSubmit(handelCreateTraningVideo)}>
        <div className="flex flex-col justify-center gap-2 mb-2">
          ...
          <input
            {...register("name", {
              required: "Please Enter Name Notification",
            })}
...
export default CreateTopNotification;
