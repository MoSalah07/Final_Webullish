...
function Card({ title, image, datesend, description, id, author, name }) {
  ...
  return (
    <Link href={`/magazine/detailsmagazine?id=${id}`} className='border-2 border-border-color flex flex-col justify-center'>
      ...
      <div className='p-4 flex flex-col justify-center gap-3 mt-4 flex-1'>
        ...
        <span className='text-secondary-gray font-normal text-sm sm:text-base md:text-lg'>{datesend}</span>
        <p className='text-text-p font-normal text-sm sm:text-base md:text-lg flex-1'>{description}</p>
      </div>
      <div className='flex items-center justify-between p-4'>
        ...
        <button onClick={(e) => handleDeleteMagazine(e, id)} className='py-[6px] px-5 rounded-primary-rounded capitalize text-sm bg-primary-red text-primary-white'>delete</button>
      </div>
    </Link>
  )
}
